...
from utils.example import Dataloader
from utils.batch import Batch
from utils.vocab import PAD
from model.TransformerCRF import TransformerCRF
...
def from_example_list(args, ex_list, device='cpu', train=True):
    ...
    input_asr_lens = [len(ex.input_idx_asr) for ex in ex_list]
    max_len = max(input_lens)
    input_ids = [ex.input_idx + [pad_idx] * (max_len - len(ex.input_idx)) for ex in ex_list]
    input_ids_asr = [ex.input_idx_asr + [pad_idx] * (max(input_asr_lens) - len(ex.input_idx_asr)) for ex in ex_list]
    batch.input_ids = torch.tensor(input_ids, dtype=torch.long, device=device)
    batch.input_ids_asr = torch.tensor(input_ids_asr, dtype=torch.long, device=device)
    batch.lengths = input_lens
    batch.lengths_asr = input_asr_lens
...
dev_dataset = Dataloader.load_dataset(dev_path, train=False)
print("Load dataset and database finished, cost %.4fs ..." % (time.time() - start_time))
print("Dataset size: train -> %d ; dev -> %d" % (len(train_dataset), len(dev_dataset)))

args.vocab_size = Dataloader.word_vocab.vocab_size
args.pad_idx = Dataloader.word_vocab[PAD]
args.num_tags = Dataloader.label_vocab.num_tags
args.tag_pad_idx = Dataloader.label_vocab.convert_tag_to_idx(PAD)


model = TransformerCRF(args, has_pos=True).to(device)
Dataloader.word2vec.load_embeddings(model.word_embed, Dataloader.word_vocab, device=device)

if args.testing:
    check_point = torch.load(open('trained_models/TransformerCRF.bin', 'rb'), map_location=device)
...
def decode(choice):
    ...
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            ...
            pred, label, loss = model.decode(Dataloader.label_vocab, current_batch)
            ...
            total_loss += loss
            count += 1
        metrics = Dataloader.evaluator.acc(predictions, labels)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        if dev_acc > best_result['dev_acc']:
            ...
            }, open('trained_models/TransformerCRF.bin', 'wb'))

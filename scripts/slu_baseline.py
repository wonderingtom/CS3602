...
if args.testing:
    check_point = torch.load(open('original_model.bin', 'rb'), map_location=device)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            # batch结构
            # inputs_id：告诉输入句子的词向量表示
            # labels：对应句子的三元组值
            # lengths：每句话在padding前的原始长度
            # tag_ids：每个单字所属标签的id，即标签的序号
            # tag_mask：padding以后向量中中真正有用的位置为1，无用的为0
            current_batch = from_example_list(args, cur_dataset, device, train=True)
            # output.shape = batch_size * utt_length * tags_num
